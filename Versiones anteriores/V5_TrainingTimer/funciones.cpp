...
void procesar_trama(char *trama, char *nuevaTrama, int *ptr_duracionActividad, int *ptr_duracionDescanso, int *ptr_totalRounds, int *ptr_totalSets, int *ptr_duracionDescansoSets, int *ptr_instanciaRutina, volatile int *ptr_segundos, int *ptr_alarmaON)
{
  ...
  switch (header[0])
  {
    case HDR_TABATA: //'T' Header para rutina de Tabata de Entrenamiento Funcional
    { 
      ...
      *ptr_duracionDescansoSets = String(subStr(copia_trama,";",6)).toInt();
      ...
      Timer1.restart();
      Timer1.start();
      Serial.println("Recibio Iniciar tabata.");
    ...
    case HDR_TIEMPOxCANTIDAD: //'C' Header para rutina de TiempoxCantidad de Entrenamiento Funcional
    {
      *ptr_instanciaRutina = TRABAJO;
      //otra forma de extraer la data particular
      char * strtokIndx; // this is used by strtok() as an index
      *ptr_duracionActividad = atoi(strtok(copia_dato,";"));    // get the first part - the string   
      *ptr_duracionDescanso = atoi(strtok(NULL, ";")); // this continues where the previous call left off
      *ptr_totalRounds = atoi(strtok(NULL, ";"));
      *ptr_totalSets = atoi(strtok(NULL, ";"));
      *ptr_duracionDescansoSets = atoi(strtok(NULL, ";"));

      cuentaDisplay(*ptr_duracionActividad - segundos);
      Timer1.restart(); //pongo el 'restart' aca porque en FINALIZADA no se porque me empezo a funcionar mal(aunque no debia)
      Timer1.start();
      *ptr_alarmaON = true;      
    ...
    { 
      *ptr_instanciaRutina = TRABAJO;
      //AMRAP -> No tiene 'rounds', serian X sets de Y minutos con un descanso entre sets de Z minutos
      //ejemplo: 2 sets de 15 minutos con 2 minutos de descanso (durante esos 15 minutos de actividad no hay rounds de menos minutos con descansos intermedios)
      char * strtokIndx; // this is used by strtok() as an index
      *ptr_duracionActividad = atoi(strtok(copia_dato,";"));    // get the first part - the string   
      *ptr_totalSets = atoi(strtok(NULL, ";"));
      *ptr_duracionDescansoSets = atoi(strtok(NULL, ";")); 
    ...
    case HDR_TKD: //'S' El celu me pide el estado de la bateria
    { //similar AMRAP, pero pa diferenciarlo por nomenclatura, usamos "Rounds" en vez de "sets" en este caso
      *ptr_instanciaRutina = TRABAJO;
