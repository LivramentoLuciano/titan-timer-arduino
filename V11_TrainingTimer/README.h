 ...
 * - (DONE en V11) Incorporacion AVANZAR/RETROCEDER_SECONDS
 *                 Fix -> Error respecto a pausa/resume al recibir un secondsForward/Backward, solucionado en 'functions' (no se inhabilita/habilita el timer, se deja como estaba)
 *                 Correccion del muestra del tiempo en display (por '_duracion' en funcion del 'instanciaRutina')
 *                 Incorpora que omita la funcion si se encuentra en 'instanciaRutina' = INIT
 *                 Reincorporacion de las alarmas sonoras, modificacion de char *mode a String, por error en dicho proceso (al hacer un if == "combat" no anda como char*)
 *                 Agregado de una alarma distinta en modo 'combat' (ejemplo, a los 10 seconds de terminar, como aviso, y dsp no esta hecho, 
 *                 pero ver de cambiar que la ultima alarma sea como la campana del final a los 0 segundos en vez del 3,2,1 de los otros modos de trabajo)
 * 
 */
