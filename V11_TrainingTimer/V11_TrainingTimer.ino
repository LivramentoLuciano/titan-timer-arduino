...
String mode; //'amrap', 'tabata', 'combat'
volatile int t_alarm = 0;
  ...
  switch (instanciaRutina){
    ...
    case TRABAJO:{
      ...
      if ((segundos >= duracionActividad - T_ALARMA_TRABAJO_SHORT) && alarmaON){  
        if ( millis()> inicioAlarma + 2*500){ //INTERVALO_BEEP){
          inicioAlarma = millis();
          tone(pinBuzzer, 1000, 500); //INTERVALO_BEEP);
        } 
      }
      //Para que en el modo 'combat' tenga un aviso 'beep-beep-beep' cuando falten 10 seconds
      if (mode == "combat" && segundos == duracionActividad - 10){
