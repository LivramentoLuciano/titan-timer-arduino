...
void procesar_trama()
{
  ...
  switch (header[0])
  {
    ...
    case HDR_SECONDS_FORWARD:
    {
      int _seconds = 0;
      strtokIndx  = strtok(NULL,";");
      _seconds = atoi(strtokIndx);

      if (instanciaRutina != INIT) secondsForward(_seconds);     
    }break;

    case HDR_SECONDS_BACKWARD:
    {
      int _secondsBack = 0;
      strtokIndx  = strtok(NULL,";");
      _secondsBack = atoi(strtokIndx);
            
      if (instanciaRutina != INIT)secondsBackward(_secondsBack);
    }break;
  }
  cG_bufferRx[0] = '\0';
  cG_nuevaTrama = false;
